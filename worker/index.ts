...
function isValidMood(mood: unknown): mood is string | undefined {
  return mood === undefined || (typeof mood === 'string' && VALID_MOODS.has(mood));
}
...
app.post('/api/heartbeat', async (c) => {
  try {
    ...
    if (!isValidSessionId(sessionId)) {
      return c.json({ error: 'Invalid sessionId: must be 8-64 alphanumeric characters' }, 400);
    }
    ...
    const allowed = await checkRateLimit(c.env.PRESENCE, `heartbeat:${sessionId}`, 10, 60);
    if (!allowed) {
  ...
  try {
    ...
    do {
      ...
      for (let i = 0; i < listResult.keys.length; i += batchSize) {
        ...
        const values = await Promise.all(batch.map((key) => c.env.PRESENCE.get(key.name)));
