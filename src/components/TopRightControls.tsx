...
export function TopRightControls({ onOpenSettings }: TopRightControlsProps) {
  const { state, setEnabled } = useAudio();
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        title={state.enabled ? 'Audio On (click to disable)' : 'Audio Off (click to enable)'}
        onClick={() => {
        ...
        style={{
          ...
          cursor: 'pointer',
          color: state.enabled ? colors.iconActive : colors.icon,
          opacity: 0.9,
