...
describe('Particle Swarm Infrastructure', () => {
  describe('API Surface & Contract', () => {
    it('exports expected functions and classes', async () => {
      const slotManagerMod = await import('./SlotManager');
      expect(slotManagerMod.SlotManager).toBeDefined();
      expect(slotManagerMod.moodCountsToUsers).toBeDefined();

      const distributionMod = await import('./swarmDistribution');
      expect(distributionMod.computeSlotTargetDirections).toBeDefined();

      const usersMod = await import('./swarmUsers');
      expect(usersMod.createUsersSignature).toBeDefined();
    });

    it('SlotManager maintains stable API signatures', () => {
      const sm = new SlotManager();
      expect(typeof sm.reconcile).toBe('function');
      expect(typeof sm.updateAnimations).toBe('function');
      expect(typeof sm.reset).toBe('function');
    });
  });

  describe('User List Signatures', () => {
    it('createUsersSignature handles empty lists', () => {
      expect(createUsersSignature([])).toBe('');
    });

    it('changes when order or contents change', () => {
      const u1: User[] = [{ id: '1', mood: 'presence' }];
      const u2: User[] = [{ id: '1', mood: 'release' }];
      expect(createUsersSignature(u1)).not.toBe(createUsersSignature(u2));
    });
  });

  describe('Swarm Distribution logic', () => {
    it('spreads active slots across sphere', () => {
      const slots: Slot[] = Array.from({ length: 4 }, (_, i) => ({
        index: i,
        state: 'active',
        userId: `u${i}`,
        mood: 'presence',
