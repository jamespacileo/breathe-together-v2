 ...
 * Uses glslFn wrapper for complex noise functions while leveraging TSL node system
 */
...
/**
 * GLSL noise functions wrapped for TSL
 * Simplex noise is complex to reimplement in pure TSL, so we use glslFn wrapper
 */
const snoiseGLSL = glslFn(`
...
export function BackgroundGradient() {
  ...
  const material = useMemo(() => {
    ...
    const colorNode = Fn(() => {
      ...
      const t1 = smoothstep(0.5, 0.9, y); // Top transition
      const t2 = smoothstep(0.25, 0.6, y); // Middle transition
      const t3 = smoothstep(0.0, 0.35, y); // Bottom transition
      ...
      const cloudUv = add(mul(uvCoord, vec2(2.0, 1.0)), vec2(mul(time, 0.015), 0.0));
      const clouds = snoiseGLSL(mul(cloudUv, 2.5));

      // Second layer of clouds moving slightly differently
      const cloudUv2 = add(
        mul(uvCoord, vec2(1.5, 0.8)),
        vec2(add(mul(time, 0.01), 50.0), mul(time, 0.003)),
      );
      const clouds2 = snoiseGLSL(mul(cloudUv2, 2.0));

      // Combine cloud layers - fade at top and bottom
      const cloudMaskBase = smoothstep(0.2, 0.55, add(mul(clouds, 0.5), mul(clouds2, 0.5)));
      const cloudMask = mul(cloudMaskBase, mul(smoothstep(0.1, 0.4, y), smoothstep(0.95, 0.6, y)));

      // Cloud color - pure warm white
      const cloudColor = vec3(1.0, 0.99, 0.97);

      // Blend clouds very subtly into sky
      const colorWithClouds = mix(skyColor, cloudColor, mul(cloudMask, 0.15));
      ...
      const colorWithVignette = mul(colorWithClouds, mix(0.97, 1.0, vignette));

      // Paper texture noise (very subtle)
      const noiseCoord = add(mul(uvCoord.x, 12.9898), mul(uvCoord.y, 78.233));
      const noise = mul(sub(fract(mul(sin(noiseCoord), 43758.5453)), 0.5), 0.008);
      const color = add(colorWithVignette, noise);
