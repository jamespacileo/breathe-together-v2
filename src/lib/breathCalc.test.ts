...
import { calculateBreathState } from './breathCalc';
import { getMoodColor } from './colors';

describe('Breathing Calculation (breathCalc)', () => {
  describe('API Surface & Contract', () => {
    it('exports expected functions', async () => {
      const breathCalc = await import('./breathCalc');
  ...
  describe('Property-Based Invariants', () => {
    ...
    it('orbit radius is always within VISUALS bounds', () => {
      const minRadius = VISUALS.PARTICLE_ORBIT_MIN;
      const maxRadius = VISUALS.PARTICLE_ORBIT_MAX;
  ...
  describe('Behavior Tests: User Outcomes', () => {
    describe('Global Breathing Synchronization', () => {
      it('all users breathe together regardless of join time', () => {
        ...
        const user1JoinTime = Date.now() / 1000;
        const user2JoinTime = user1JoinTime + 10; // 10 seconds later

        // User 1 checks phase 30 seconds after their join
        const user1CheckTime = user1JoinTime + 30;
        // User 2 checks phase 20 seconds after their join (same absolute UTC time)
        const user2CheckTime = user2JoinTime + 20;
      ...
      it('different absolute times produce different phases', () => {
        ...
        const phase2 = calculateBreathState(baseTime + 1).breathPhase; // 1 second later
      ...
      it('breathing cycle completes in exactly 19 seconds', () => {
        ...
        const endTime = startTime + 19; // 19 seconds later
