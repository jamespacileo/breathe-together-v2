...
export function usePerformanceMonitor(): PerformanceState {
  ...
  useEffect(() => {
    ...
    if (perf < THRESHOLDS.LOW && qualityLevel !== 'low') {
      ...
      newLevel = 'high';
    } else if (perf >= THRESHOLDS.LOW && perf <= THRESHOLDS.HIGH && qualityLevel !== 'medium') {
      newLevel = 'medium';
    }
  ...
  const recommendedParticleCount = PARTICLE_COUNTS[qualityLevel as keyof typeof PARTICLE_COUNTS];
