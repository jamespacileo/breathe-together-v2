...
describe('Material Visibility Tests', () => {
  describe('Globe Material Opacity', () => {
    it('globe main material is opaque (opacity = 1.0)', () => {
      // OUTCOME: Globe is fully visible
      ...
      expect(globeMaterial.opacity).toBe(1.0);
      expect(globeMaterial.transparent).toBe(false);
      expect(globeMaterial.visible).toBe(true);
    });

    it('globe overlay materials maintain minimum visibility', () => {
      // OUTCOME: Atmosphere overlays don't completely hide globe
      const minOpacity = 0.08; // Default atmosphere opacity
      const maxOpacity = 0.5; // Maximum reasonable overlay
    ...
    it('frosted glass overlay has visible opacity range', () => {
  ...
  describe('Particle Material Opacity', () => {
    it('particle swarm shards are never invisible', () => {
      // OUTCOME: All particles have visible opacity
      const particleOpacity = 0.9; // Default from ParticleSwarm
    ...
    it('mood-colored particles maintain visibility', () => {
      // OUTCOME: All mood colors are visible
      const moodOpacities = {
        gratitude: 0.9,
        presence: 0.9,
        release: 0.9,
      ...
      for (const [_mood, opacity] of Object.entries(moodOpacities)) {
    ...
    it('atmospheric particles have subtle but visible opacity', () => {
      ...
      const atmosphericOpacity = 0.6; // Default for atmospheric particles
  ...
  describe('Cloud Material Opacity', () => {
    it('all cloud configurations have visible opacity', () => {
      // OUTCOME: No clouds are invisible
      for (const cloud of CLOUD_CONFIGS) {
        expect(cloud.opacity).toBeGreaterThan(0);
        expect(cloud.opacity).toBeLessThanOrEqual(1.0);

        // Clouds should be subtle but visible
        expect(cloud.opacity).toBeGreaterThan(0.2);
        expect(cloud.opacity).toBeLessThan(0.5);
      }
    });
  ...
  describe('Material Color Brightness', () => {
    ...
    it('no materials use pure white on light background (invisible)', () => {
      ...
      const lightBackground = new THREE.Color('#f5f0e8');
  ...
  describe('Transparent Materials Validation', () => {
    it('transparent materials have reasonable opacity ranges', () => {
      // OUTCOME: Transparency is intentional, not accidental
      const transparentMaterials = [
        { name: 'frosted-glass', opacity: 0.9, minExpected: 0.7, maxExpected: 0.95 },
        { name: 'atmosphere', opacity: 0.08, minExpected: 0.05, maxExpected: 0.15 },
        { name: 'cloud', opacity: 0.35, minExpected: 0.2, maxExpected: 0.5 },
      ];
    ...
    it('no materials have opacity of exactly 0 (completely invisible)', () => {
      // OUTCOME: Prevents accidental invisible objects
      const allOpacities = [
        1.0, // Globe
        0.9, // Particles
        0.08, // Atmosphere
  ...
  describe('Material Visibility Property', () => {
    it('all materials have visible=true by default', () => {
      // OUTCOME: Materials render unless explicitly hidden
      const materials = [
  ...
  describe('Material Opacity Mutation Protection', () => {
    it('detects if globe opacity is accidentally set to 0', () => {
      // OUTCOME: Mutation test - globe should be visible
      const validGlobeOpacity = 1.0;
    ...
    it('detects if particle opacity drops below threshold', () => {
      // OUTCOME: Mutation test - particles should be visible
      const validParticleOpacity = 0.9;
    ...
    it('detects if all cloud opacities become 0', () => {
      ...
      const hasVisibleClouds = cloudOpacities.some((opacity) => opacity > 0.2);
